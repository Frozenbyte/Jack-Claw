	...
	// Rendertarget stuff
	{
		...
		// draws lights to fakeTexture
		{
			...
			if (!data->fbo->validate()) {
				LOG_WARNING("renderTargets: renderTarget validate failed with fakeTexture\n");
				data->fbo->disable();
				return;
			}
		...
		if(data->renderGlows)
		{
			...
			// Material self illuminations to glow texture
			{
				...
				if (!data->fbo->validate()) {
					LOG_WARNING("renderTargets: renderTarget validate failed with glowTexture2\n");
					data->fbo->disable();
					return;
				}
				...
				{
					{
						...
						if (!data->fbo->validate()) {
							LOG_WARNING("renderTargets: renderTarget validate failed with glowTexture1\n");
							data->fbo->disable();
							return;
						}
						glDepthMask(GL_FALSE);
		...
		if(data->renderOffsets && data->offsetTexture)
		{
			...
			if (!data->fbo->validate()) {
				LOG_WARNING("renderTargets: renderTarget validate failed with offsetTexture\n");
				data->fbo->disable();
				return;
			}
			glDepthMask(GL_FALSE);
		...
		if(data->renderGlows)
		{
			...
			data->fbo->setRenderTarget(data->renderTexture);
			if (!data->fbo->validate()) {
				LOG_WARNING("renderTargets: renderTarget validate failed with renderTexture\n");
				data->fbo->disable();
				return;
			}
			glViewport(data->viewport.x, data->viewport.y, data->viewport.w, data->viewport.h);
