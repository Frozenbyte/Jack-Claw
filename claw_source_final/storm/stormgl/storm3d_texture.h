...
protected:
	...
	uint32_t tex_identity;				// Used if no sharing wanted
	uint32_t texloadflags;				// Onfy for loaded textures (NoCompress etc.)
...
public:
	...
	bool IsIdenticalWith(const char *_filename,uint32_t texloadflags,uint32_t tex_identity) const;
	...
	void CopyTextureTo8BitSysMembuffer(uint8_t *sysbuffer);	// Creates grayscale color from green color component
	...
	Storm3D_Texture(Storm3D *Storm3D2,const char *_filename,
		uint32_t texloadflags,uint32_t tex_identity=0,
...
public:
	...
	// Creation/delete
	Storm3D_Texture_Video(Storm3D *Storm3D2,const char *_filename,uint32_t tex_identity=0);
	~Storm3D_Texture_Video();
};
