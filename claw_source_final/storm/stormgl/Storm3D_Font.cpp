...
void Storm3D_Font::SetFont(const char *face_, int width_, int height_, bool bold_, bool italic_)
{
	...
	if(strcasecmp(face_, "CrimeFighter BB") == 0)
	{
		font_file = "./data/fonts/unicode/";
		font_file.append("crimfbrg");
	}
	// HACK HACK HACK
	// Someone is trying to load a font with null string as filename and zero height, and yet it is used!
	// Searching for "verdEna" is equally stupid...
	else if(strcasecmp(face_, "") == 0 || strcasecmp(face_, "Verdena") == 0)
	{
		font_file = getFontDirectory();
	...
	if (*face_ == '\0') {
		...
		height_ = 14;
	} else if(strcasecmp(face_, "CrimeFighter BB") == 0) {
		font_file.append("crimfbrg");
	} else if(strcasecmp(face_, "Verdana") == 0 || strcasecmp(face_, "Verdena") == 0) {
	...
	if (font)
		...
		LOG_WARNING(strPrintf("Font (%s) load failed: %s\n", font_file.c_str(), TTF_GetError()).c_str());
	}
