#ifndef WINUSER_RAWINPUT
#define WINUSER_RAWINPUT
...
typedef struct tagRAWHID {
	uint32_t dwSizeHid;    // byte size of each report
	uint32_t dwCount;      // number of input packed
	uint8_t bRawData[1];
...
#ifdef _WIN64
#define RAWINPUT_ALIGN(x)   (((x) + sizeof(Quint16_t) - 1) & ~(sizeof(Quint16_t) - 1))
#else   // _WIN64
#define RAWINPUT_ALIGN(x)   (((x) + sizeof(uint32_t) - 1) & ~(sizeof(uint32_t) - 1))
#endif  // _WIN64
...
#endif  // !defined(_WINUSER_) && !defined (_WINUSER_H)

#endif  // _MSC_VER

#endif  // WINUSER_RAWINPUT
