...
Storm3D::~Storm3D()
{
	...
	// FIXME: May now leak memory, but otherwise crashes on Windows
#ifndef _MSC_VER
...
bool Storm3D::SetRenderTarget(Storm3D_Texture *newtarget,int map)
{
	...
	if (!newtarget->IsCube())
	{
		if(newtarget->texhandle)
		{
			...
			if (!fbo->validate()) {
				LOG_WARNING(strPrintf("setRenderTarget failed, texture format: %x\n", newtarget->texfmt).c_str());
				LOG_WARNING(strPrintf("size: %dx%d type:%d\n", newtarget->width, newtarget->height, newtarget->textype).c_str());
				fbo->disable();
				return false;
			}
	...
	else
	{
		...
		return false; // due to unimplemented
		// Get cube face
	}
	return true;
}
...
bool Storm3D::RenderSceneToArray( IStorm3D_Scene * stormScene, unsigned char * dest, int width, int height )
{
	...
	{
        LOG_ERROR("Storm: Couldn't create render target.");
	}
