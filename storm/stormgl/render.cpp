...
uintptr_t applyFVF(VxFormat fmt, uintptr_t size) {
	...
	if (fmt & D3DFVF_XYZ) {
		...
		LOG_WARNING("strange: neither D3DFVF_XYZ nor D3DFVF_XYZRHW\n");
	}
	...
	if (fmt) {
		LOG_WARNING(strPrintf("strange: fmt=%08x\n", fmt).c_str());
		backtrace();
	}

	if (ptr != size) {
#ifdef __x86_64
		LOG_WARNING(strPrintf("applyFVF: ptr(%ld) != size(%ld)\n", ptr, size).c_str());
#else
		LOG_WARNING(strPrintf("applyFVF: ptr(%d) != size(%d)\n", ptr, size).c_str());
#endif
		backtrace();
	}
...
void renderUP(VxFormat fmt, GLenum type, int count, int size, char *vx) {
	...
	if (ptr != size) {
		LOG_WARNING(strPrintf("strange: ptr(%d) != size(%d)\n", ptr, size).c_str());
		backtrace();
	}
...
void dumpD3DXMatrix(const D3DXMATRIX &mat) {
	for (unsigned int i = 0; i < 4; i++) {
        LOG_DEBUG(strPrintf("%f\t%f\t%f\t%f\n", mat.m[i][0], mat.m[i][1], mat.m[i][2], mat.m[i][3]).c_str());
	}
}
...
void Framebuffer::setRenderTarget(GLuint tex, GLuint width, GLuint height, GLenum textarget) {
    ...
    // no match?
	if (depthstencil == 0) {
		LOG_DEBUG(strPrintf("create new depthstencil buffer size %dx%d\n", width, height).c_str());
...
void Framebuffer::setRenderTarget(boost::shared_ptr<glTexWrapper> tex, GLenum textarget) {
	...
	if (tex->fmt != GL_RGBA8) {
		LOG_WARNING(strPrintf("setRenderTarget: tex->fmt(0x%x) != GL_RGBA8\n", tex->fmt).c_str());
		backtrace();
	}
	tex->bind();
	...
	if (tempW != tex->getWidth() || tempH != tex->getHeight()) {
		LOG_WARNING(strPrintf("Framebuffer::setRenderTarget: texture size has changed! Should be (%dx%d), is (%dx%d)\n", tex->getWidth(), tex->getHeight(), tempW, tempH).c_str());
		backtrace();
	}
	if ((GLenum) tempFmt != tex->getFmt()) {
		LOG_WARNING(strPrintf("Framebuffer::setRenderTarget: texture format has changed! Should be %x, is %x\n", tex->getFmt(), tempFmt).c_str());
		backtrace();
	}
...
void Framebuffer::setRenderTarget(boost::shared_ptr<glTexWrapper> tex, boost::shared_ptr<glTexWrapper> depthstencil, GLenum textarget) {
	...
	if (depthstencil->fmt != GL_DEPTH24_STENCIL8_EXT) {
		LOG_WARNING(strPrintf("setRenderTarget: depthstencil->fmt(0x%x) != GL_DEPTH24_STENCIL8_EXT\n", depthstencil->fmt).c_str());
		backtrace();
	}

	if (tex->getWidth() != depthstencil->getWidth() || tex->getHeight() != depthstencil->getHeight()) {
		LOG_WARNING(strPrintf("setRenderTarget: tex and depthstencil size mismatch: %dx%dx vs %dx%d\n", tex->getWidth(), tex->getHeight(), depthstencil->getWidth(), depthstencil->getHeight()).c_str());
		backtrace();
	}
	...
	if (tempW != depthstencil->getWidth() || tempH != depthstencil->getHeight()) {
		LOG_WARNING(strPrintf("Framebuffer::setRenderTarget: depthstencil size has changed! Should be (%dx%d), is (%dx%d)\n", depthstencil->getWidth(), depthstencil->getHeight(), tempW, tempH).c_str());
		backtrace();
	}

	if ((GLenum) tempFmt != depthstencil->getFmt()) {
		LOG_WARNING(strPrintf("Framebuffer::setRenderTarget: texture format has changed! Should be %x, is %x\n", depthstencil->getFmt(), tempFmt).c_str());
		backtrace();
	}
	glBindTexture(textarget, 0);
...
bool Framebuffer::validate() {
	if (activeFBO == this) {
		...
		if (ret != GL_FRAMEBUFFER_COMPLETE_EXT) {
			LOG_WARNING(strPrintf("framebuffer not complete at %d: %x\n", __LINE__, ret).c_str());
			*((char *) NULL) = '\0';
			backtrace();
...
boost::shared_ptr<glTexWrapper> glTexWrapper::rgbaTexture(GLint w, GLint h) {
	...
	if (tex->fmt != (GLenum) tempFmt || tempH != h || tempW != w) {
		LOG_WARNING(strPrintf("glTexWrapper::rgbaTexture: wanted %x, got %x\n", tex->fmt, tempFmt).c_str());
		LOG_WARNING(strPrintf("attempted size: %dx%d\ntexture size: %dx%d\n", w, h, tempH, tempW).c_str());
		tex->fmt = tempFmt;
	}
...
boost::shared_ptr<glTexWrapper> glTexWrapper::depthStencilTexture(GLint w, GLint h) {
	...
	if (tex->fmt != (GLenum) tempFmt) {
		LOG_WARNING(strPrintf("glTexWrapper::depthStencilTexture: wanted %x, got %x\n", tex->fmt, tempFmt).c_str());
		tex->fmt = tempFmt;
	}
...
void glTexWrapper::bind() {
	...
	if (tempW != width || tempH != height) {
		LOG_WARNING(strPrintf("glTexWrapper::bind(): texture size has changed! Should be (%dx%d), is (%dx%d)\n", width, height, tempW, tempH).c_str());
		backtrace();
	}
	...
	if ((GLenum) tempFmt != fmt) {
		LOG_WARNING(strPrintf("glTexWrapper::bind(): texture format has changed! Should be %x, is %x\n", fmt, tempFmt).c_str());
		backtrace();
	}
...
bool checkGlErrors(const char *file, int line)
{
	if (gl_initialized) {
		int err = glGetError();
		if (err != GL_NO_ERROR) {
			LOG_WARNING(strPrintf("gl error at %s:%d: %x %s\n", file, line, err, gluErrorString(err)).c_str());
			backtrace();
			return true;
		}
	}
