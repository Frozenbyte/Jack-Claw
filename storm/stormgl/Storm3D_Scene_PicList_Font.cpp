...
void Storm3D_Scene_PicList_Font::Render()
{
	...
	if(font->font)
	{
		...
		if (!tex)
		{
			...
			if(uniText)
			{
				...
				// apparently wchar_t is not always 16 bits
				if (sizeof(wchar_t) == 2) {
					buffer = TTF_RenderUNICODE_Blended(font->GetFont(), (Uint16*)uniText, col2);
				} else {
					size_t len = wcslen(uniText);
					boost::scoped_array<Uint16> tmp(new Uint16[len + 1]);
					for (size_t i = 0; i < len + 1; ++i)
						tmp[i] = uniText[i] & 0xffff;
					buffer = TTF_RenderUNICODE_Blended(font->GetFont(), tmp.get(), col2);
				}
			}
			else if(text)
			{
	...
	else if(text)
	{
		...
		LOG_WARNING("no font and no text\n");
	}
}
