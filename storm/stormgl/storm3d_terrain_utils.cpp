...
void setTracing(bool tracing_) {
	if (tracing != tracing_) {
		tracing = tracing_;
		LOG_WARNING(strPrintf("Tracing %s\n", tracing_ ? "enabled" : "disabled").c_str());
		if (tracing_) {
			LOG_WARNING("Current shaders:\n");
			activeShaderNames();
		}
	}
}
...
void activeShaderNames() {
	LOG_WARNING(strPrintf("vertex: %s\npixel: %s\n", activeVertexShader.c_str(), activePixelShader.c_str()).c_str());
}
...
void PixelShader::disable() {
	...
	if (tracing) {
		LOG_WARNING("pixel shader disabled\n");
	}
}
...
void VertexShader::disable() {
	...
	if (tracing) {
		LOG_WARNING("vertex shader disabled\n");
	}
}
...
void PixelShader::createPixelShader(const std::string &name) {
	...
	if (errorPos != -1) {
		LOG_WARNING(strPrintf("error creating shader %s: %s\n", name.c_str(), glGetString(GL_PROGRAM_ERROR_STRING_ARB)).c_str());
	}
...
void VertexShader::apply() const
{
	...
	if (tracing) {
		LOG_WARNING(strPrintf("activated vertex shader %s\n", nm.c_str()).c_str());
	}
	applyDeclaration();
}
...
void PixelShader::apply() const
{
	...
	if (tracing) {
		LOG_WARNING(strPrintf("activated pixel shader %s\n", nm.c_str()).c_str());
	}
}
...
{
	if (vertexOffset != 0) {
		LOG_WARNING("IndexBuffer::render warning: vertexOffset != 0\n");
		backtrace();
	}
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffer);
