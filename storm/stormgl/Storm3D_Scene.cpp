...
Storm3D_Scene::~Storm3D_Scene()
{
	...
	for( int i = 1; i < 9; i++ )
	{
		...
		LOG_WARNING(strPrintf("%i: %d \n", i, bottlenecks[i]).c_str());
	}
#endif
}
...
void Storm3D_Scene::RenderSceneWithParams(bool flip,bool disable_hsr, bool update_time, bool render_mirrored, IStorm3D_Texture *target)
{
	...
	if (flip) {
		...
		if (nCount > 0) {
			LOG_WARNING(strPrintf("begin experiment, %d cycles\n", nCount).c_str());
			...
			bottlenecks[value]++;
			LOG_WARNING(strPrintf("GPU Bottleneck value: %lu cycles: %lu\n", value, cycles).c_str());
			LOG_WARNING(strPrintf("Bottleneck : %s\n", bname).c_str());
			delete[] bname;
		} else {
