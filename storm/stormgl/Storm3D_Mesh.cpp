...
void Storm3D_Mesh::ReBuild()
{
	...
	if((update_vx) && (!bone_weights))
	{
		...
		{
			// FIXME: this if BUGGY! should not be necessary
			LOG_WARNING(strPrintf("vbuf %d %d\n", vbuf, glIsBuffer(vbuf)).c_str());
			LOG_WARNING(strPrintf("size: %d vertex amount: %d %d\n", temp, vertex_amount, vertex_amount*sizeof(VXFORMAT_TC2)).c_str());
		} else {
			// Typecast (to simplify code)
			VXFORMAT_TC2 *p=(VXFORMAT_TC2*) glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
			if (p == NULL) {
				LOG_WARNING("MapBuffer failed\n");
				glErrors();
				return;
			}
