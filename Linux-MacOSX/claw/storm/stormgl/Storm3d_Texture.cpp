...
//! Constructor
Storm3D_Texture::Storm3D_Texture(Storm3D *s2,const char *_filename,uint32_t _texloadflags,
								 uint32_t _tex_identity, const void *data, size_t data_size) :
	decentAlpha(false),
	Storm3D2(s2),
	texhandle(0),
	...
	*/
	...
	if (data == NULL)
	{
		...
		if(file)
		{
			...
			LOG_ERROR(strPrintf("Failed to load texture %s", filename).c_str());
			return ;
		}
	} else {
		fileData.reset(new char[data_size]);
	...
	if (tmp == NULL) {
		LOG_ERROR(strPrintf("Failed to load texture %s: %s", filename, IMG_GetError()).c_str());
		return ;
	}
		...
		for (int i = 0; i < mipmaps; i++) {
			...
			case 2:
				if (fmt == GL_DU8DV8_ATI) {
					...
					LOG_WARNING(strPrintf("Strange fmt-mult combination: %d %x\n", fmt, mult).c_str());
				}
			...
			case 4:
				...
				break;
			default:
				LOG_WARNING("Bad DDS texture bpp\n");
				break;
			};
			int size = w * h * mult;
...
bool Storm3D_Texture::loadDDS(const char *filename, const char *fileData) {
	if (memcmp(fileData, "DDS ", 4) != 0) {
		//LOG_ERROR(strPrintf("invalid DDS file %s", filename).c_str());
		return false;
	}

	Uint32 surfaceDestLen = SDL_SwapLE32(*(Uint32 *) (fileData + 0x04));
	if (surfaceDestLen != 124) {
		//LOG_ERROR(strPrintf("invalid DDS file %s: size mismatch (%d)", filename, surfaceDestLen).c_str());
		return false;
	}
	...
	if ((caps & 0x1007) == 0) {
		LOG_WARNING(strPrintf("%s: strange caps: %08x\n", filename, caps).c_str());
		return false;
	}
	...
	case 0x4: // DDPF_FOURCC
		...
		case 0x35545844: // DXT5
			...
			break;
		default:
			LOG_ERROR(strPrintf("unknown fourcc: %08x\n", fourcc).c_str());
			return false;
		}
	...
	case 0x80000:
		...
		break;
	default:
		LOG_ERROR(strPrintf("%s\nunknown pixelformat: %08x\n", filename, pixelFormat).c_str());
		return false;
	}
	...
	auto_release(true)
{
	...
	if (width>mwidth) 
	{
		LOG_ERROR("GFX card doesn't support textures this big.");
		width=mwidth;
	}
	if (height>mheight) 
	{
...
*/
bool Storm3D_Texture::IsIdenticalWith(const char *_filename,uint32_t _texloadflags,uint32_t _tex_identity) const
{
