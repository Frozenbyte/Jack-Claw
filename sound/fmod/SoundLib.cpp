...
struct StormStream: public IStorm3D_Stream
{
...
#ifdef _WIN32
	CRITICAL_SECTION stream_critical_section;
	bool stream_critical_section_inited;
#else
	...
	:	stereo(stereo_),
		...
		position(0),
#ifdef _WIN32
		stream_critical_section_inited(false),
#endif
		updateTime(0),
		sampleSize(0),
		active(false),
	...
	{
#ifdef _WIN32
		if (!stream_critical_section_inited)
		{
			stream_critical_section_inited = true;
			InitializeCriticalSection(&stream_critical_section);
		}
		else
		{
			assert(!"StormStream - stream_critical_section_inited already set.");
		}
#else
	...
	~StormStream()
	{
		...
		{
			stream_critical_section_inited = false;
			DeleteCriticalSection(&stream_critical_section);
		}
		else
		{
			assert(!"StormStream::~StormStream - stream_critical_section_inited was not set.");
		}
#else
	...
	{
#ifdef _WIN32
		EnterCriticalSection(&stream_critical_section);
#else
		SDL_LockMutex(lock);
#endif
...
#ifdef _WIN32
		LeaveCriticalSection(&stream_critical_section);
#else
...
#ifdef _WIN32
		EnterCriticalSection(&ptr->stream_critical_section);
#else
...
#ifdef _WIN32
	LeaveCriticalSection(&ptr->stream_critical_section);
#else
